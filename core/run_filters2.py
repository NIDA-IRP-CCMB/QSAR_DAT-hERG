...
# core_dir = '/home/khlee/cheminformatics/tocheck/core'
...
def main(argv):
    try:
        ...
        print(sys.argv[
                  0] + ' -t <tsv_file>  -p <target_protein> -a <assaydefinition> -o <output_dir> -b <output_file_basename> -s <standard_type>')
        sys.exit(2)
            ...
            if assaydefinition != "clamp":
                if assaydefinition != "binding":
                    if assaydefinition != "others":
                        print(
                            "error: Need to specify assaydefinition DAT (uptake/inhibitor) or hERG(clamp/binding/others)")
                        sys.exit()
