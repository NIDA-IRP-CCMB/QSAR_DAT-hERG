...
def filter_assaydefinition_DAT(in_lines, kickout=False, Verbose=False):
    # filter for displacement assay data

    fin = conf_dir+'/assaydefinition_DAT_inhibitor.txt'
    df = pd.read_table(fin,names=['keys','text'])  
    # nkeys=len(df['keys'].unique())
    # df['text'][df['keys']==df['keys'].unique()[3]]
    selection = list(df['text'])

    in_lines = in_lines[in_lines['description'].apply(lambda x: any([s in x for s in selection]))]
    
    if Verbose:
        print('Number of compounds after Displacement Assay filter: ', len(in_lines))

    return in_lines.reset_index(drop=True)

def filter_assaydefinition_DAT2(in_lines, kickout=False, Verbose=False):
    # filter for displacement assay data
    ...
    in_lines = in_lines[in_lines['description'].apply(lambda x: all([s not in x for s in selection]))]
...
def filter_assaydefinition_DAT_uptake(in_lines, kickout=False, Verbose=False):
    # filter for displacement assay data

    fin = conf_dir+'/assaydefinition_DAT_uptake.txt'
    df = pd.read_table(fin,names=['keys','text'])  
    selection = list(df['text'])
...
def filter_assaydefinition_DAT_uptake2(in_lines, kickout=False, Verbose=False):
    # filter for displacement assay data
...
def filter_assaydefinition_hERG(in_lines, filterfile, key, kickout=False,
                                Verbose=False):
    # filter for displacement assay data

    # fin = conf_dir+'/assaydefinition_hERG_other.txt'
    df = pd.read_table(filterfile, names=['keys', 'text'])
    selection = list(df['text'])

    #     selection = ['Inhibition of human ERG', \
    ...
    in_lines_out = in_lines[
        in_lines['description'].apply(lambda x: all([s not in x for s in selection]))]
    # in_lines = in_lines[in_lines['description'].apply(lambda x: any([s == x for s in selection]))]
...
def filter_assaydefinition_hERG2(in_lines, kickout=False, Verbose=False):
    # filter for displacement assay data
    
    fin= conf_dir+'/assaydefinition_hERG.txt'
    df = pd.read_table(fin,names=['keys','text'])  
    selection = list(df['text'])

    #in_lines = in_lines[in_lines['description'].apply(lambda x: any([s in x for s in selection]))]
    in_lines = in_lines[in_lines['description'].apply(lambda x: all([s != x for s in selection]))]
